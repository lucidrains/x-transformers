...
What originally started off as <a href="https://arxiv.org/abs/2112.05682">a short paper</a> from Markus Rabe culminated as a practical integrated solution on CUDA, named <a href="https://arxiv.org/abs/2205.14135">Flash Attention</a> by <a href="https://tridao.me/">Tri Dao</a>.

The technique processes the attention matrix in tiles, only keeping track of the running softmax and exponentiated weighted sums. By recomputing on the backwards pass in a similar fashion, one is able to keep the memory linear with respect to sequence length. This allows a lot of recent models  to be able to reach for longer context lengths without worrying about the memory bottleneck.

Other engineering decisions made by Tri Dao led to its enormous success, namely minimizing HBM accesses so that both the forwards and backwards outperforms naive attention. In other words, flash attention is both more memory efficient and faster, making it a necessity for training transformers.
...
<a href="https://ai.facebook.com/blog/large-language-model-llama-meta-ai/">Llama</a> was trained using Flash Attention. The only reason not to use it is if you are using some feature that requires gradients from the attention matrix (dynamic positional bias, talking heads, residual attention).

You can turn it on in this repository by setting `attn_flash` to `True` and enjoy the immediate memory savings + increase in speed.
